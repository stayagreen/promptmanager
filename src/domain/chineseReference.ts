...
const phraseTranslations: Array<[string, string]> = [
  ...
  ["REFERENCE-IMAGE PRODUCT TASK", "参考图产品任务"],
  ["REFERENCE-IMAGE PRODUCT IDENTITY LOCK", "参考图产品身份锁定"],
  ["Use every supplied product reference image as visual evidence.", "把所有已上传的产品参考图都作为视觉依据。"],
  [
    "The primary reference image defines overall identity; supplementary images clarify angles, materials, joints, and construction.",
    "主参考图定义产品整体身份；补充图用于说明角度、材质、连接件和结构细节。",
  ],
  ["STRICT PRODUCT IDENTITY LOCK.", "严格产品身份锁定。"],
  ["HIGH PRODUCT IDENTITY LOCK.", "高度产品身份锁定。"],
  ["CONTROLLED PRESENTATION OPTIMIZATION.", "可控展示优化。"],
  ["CREATIVE INTERPRETATION WITH IDENTITY RETENTION.", "在保留产品身份前提下进行创意演绎。"],
  ["TASK", "任务"],
  ["ALLOWED PRESENTATION CHANGES", "允许的展示变化"],
  ["SCENE STYLE", "场景风格"],
  ["PHOTOGRAPHY DIRECTION", "摄影方向"],
  ["OUTPUT FORMAT", "输出格式"],
  ["GLOBAL RESTRICTIONS", "全局限制"],
  ["ADDITIONAL REQUIREMENTS", "附加要求"],
  ["USAGE NOTE", "使用说明"],
  ["BOTANICAL INSPIRATION LAB LOCK", "插花灵感实验室锁定"],
  ["HIGHEST-PRIORITY EXECUTION", "最高优先级执行"],
  ["BOTANICAL DESIGN FAMILY SELECTION", "植物花型家族选择"],
  ["ANTI-REPETITION LOCK", "反重复锁定"],
  ["SAME LAMP IDENTITY CARD", "同一盏灯身份卡"],
  ["MATERIAL AND COLOR SYSTEM", "材质与颜色系统"],
  ["BLUE LIMITATION", "蓝色限制"],
  ["MOBILE STILL-LIFE NATURALNESS", "手机静物自然感"],
  ["ANTI-GREY CLARITY", "反灰雾通透"],
  ["STILL-LIFE DETAILS", "静物细节"],
  ["NO TEXT RULE", "无文字规则"],
  ["Image 1 - Single Lamp Cover", "第 1 张 - 单灯封面"],
  ["Image 2 - Daylight Off", "第 2 张 - 白天关灯"],
  ["Image 3 - Daylight On", "第 3 张 - 白天开灯"],
  ["Image 4 - Flower Form Detail", "第 4 张 - 花型细节"],
  ["Image 5 - Quiet Home Scene", "第 5 张 - 安静家居场景"],
  ["Botanical Design Family", "植物花型家族"],
  ["Product archive:", "产品档案："],
  ["Background replacement is allowed.", "允许更换背景。"],
  ["Keep the original background character when possible.", "尽可能保留原始背景气质。"],
  [
    "A different camera angle is allowed, but product geometry must remain accurate.",
    "允许改变拍摄角度，但产品几何结构必须保持准确。",
  ],
  ["Keep a camera angle close to the primary reference image.", "保持接近主参考图的拍摄角度。"],
  ["Lighting and exposure optimization are allowed.", "允许优化光线和曝光。"],
  ["Do not materially change the original illumination behavior.", "不要实质性改变原始发光方式。"],
  [
    "A small number of relevant props may be added without obscuring the product.",
    "可以少量增加相关道具，但不能遮挡产品。",
  ],
  ["Do not add decorative props.", "不要增加装饰道具。"],
  [
    "The visible power cable may be tidied or routed more cleanly, but must not imply a wireless product.",
    "可整理可见电源线或让走线更干净，但不能暗示产品是无线的。",
  ],
  ["Preserve visible cables and power connections.", "保留可见电线和电源连接。"],
  [
    "Use the uploaded reference images as the product identity source.",
    "使用上传的参考图作为产品身份来源。",
  ],
  [
    "Generate a single image divided into a seamless 2×2 product photography storyboard grid",
    "生成一张无缝 2×2 四宫格产品摄影分镜图",
  ],
  [
    "using the uploaded reference images as the product identity source.",
    "并以上传的参考图作为产品身份来源。",
  ],
  ["All four panels must show the EXACT SAME reference lamp.", "四个画面必须展示同一盏参考图灯具。"],
  [
    "Only camera angle, lighting condition, distance, background, and photographic composition may change within the allowed presentation rules.",
    "在允许的展示规则内，只能改变拍摄角度、光线状态、距离、背景和摄影构图。",
  ],
  [
    "The final image must be a true seamless 2×2 grid:",
    "最终图像必须是真正无缝的 2×2 四宫格：",
  ],
  ["no spacing", "无间距"],
  ["no gutters", "无分栏缝"],
  ["no margins", "无边距"],
  ...
  ["no watermark", "无水印"],
  [
    "The reference images are not embedded in this text prompt.",
    "参考图不会嵌入到这段文字提示词中。",
  ],
  [
    "Upload the corresponding product images to the image-generation platform together with this prompt and set the primary image as the main product reference.",
    "使用时请把对应产品图片和这段提示词一起上传到生图平台，并把主图设为最高优先级产品参考。",
  ],
  [
    "Create a five-image independent vertical set inspired by a botanical floral-form lighting object.",
    "生成一套由植物花型灯具启发的 5 张独立竖版图片。",
  ],
  [
    "Generate five separate images sequentially from Image 1 through Image 5.",
    "按第 1 张到第 5 张的顺序连续生成五张独立图片。",
  ],
  [
    "Do not create a collage, grid, long image, multi-panel canvas, or contact sheet.",
    "不要生成拼图、网格、长图、多画面画布或联系表。",
  ],
  [
    "Each image must be an independent 3:4 vertical smartphone still-life photograph.",
    "每张图都必须是独立的 3:4 竖版手机静物摄影。",
  ],
  [
    "All five images must show the exact same lamp design and the selected lighting type.",
    "五张图必须展示同一款灯具设计，并保持所选灯具类型不变。",
  ],
  [
    "Before generation, internally choose one uncommon Botanical Design Family for this run and keep it consistent across all five images.",
    "生成前先在内部为本轮选择一个不常见的植物花型家族，并在五张图中保持一致。",
  ],
  [
    "Translate the botanical inspiration into the lamp's shade, diffuser, support, joint, base, or mounting language; never attach real flowers to the lamp.",
    "把植物灵感转译到灯罩、扩散罩、支架、连接件、底座或安装结构中；绝不能把真实花朵插到灯上。",
  ],
  ["Generate exactly five independent images in this order:", "严格按以下顺序生成五张独立图片："],
  ["Each image must be 3:4 vertical.", "每张图必须是 3:4 竖版。"],
  ["Choose one uncommon family from this list:", "从以下列表中选择一个不常见家族："],
  ["A. Bell Shade Family", "A. 垂铃花罩家族"],
  ["B. Star / Clematis Family", "B. 星形 / 铁线莲家族"],
  ["C. Folded Iris Family", "C. 折瓣鸢尾家族"],
  ["D. Layered Petal Family", "D. 层叠花瓣家族"],
  ["E. Pod / Seed Family", "E. 种荚 / 种子家族"],
  ["F. Cluster Diffuser Family", "F. 小簇扩散罩家族"],
  ["G. Line Botanical Family", "G. 线条植物家族"],
  ["H. Orchid Variant Family", "H. 兰花变体家族"],
  ["I. Lily Variant Family", "I. 百合变体家族"],
  ["J. Lotus / Water Plant Family", "J. 荷与水生植物家族"],
  ["Aspect Ratio", "图片比例"],
  [
